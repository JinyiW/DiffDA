...
def train_source(args):
    ...
    optimizer = op_copy(optimizer) # copy optimizer
    ...
    while iter_num < max_iter: 
        ...
        iter_num += 1
        lr_scheduler(optimizer, iter_num=iter_num, max_iter=max_iter) # update learning rate

        inputs_source, labels_source = inputs_source.cuda(), labels_source.cuda() # move to gpu
        outputs_source = netC(netB(netF(inputs_source))) # forward pass
        classifier_loss = CrossEntropyLabelSmooth(num_classes=args.class_num, epsilon=args.smooth)(outputs_source, labels_source)           
        
        optimizer.zero_grad() # clear gradients for this training step
        classifier_loss.backward() # backward pass
        optimizer.step() # update network parameters

        if iter_num % interval_iter == 0 or iter_num == max_iter: # print loss
            netF.eval()
            netB.eval()
            netC.eval()
...
if __name__ == "__main__":
    ...
    for i in range(len(names)): # test on all target datasets
        if i == args.s:
            continue
        args.t = i
