...
def save_features(args,if_save_source=True):
## set base network
    if args.net[0:3] == 'res':
    ...
    if if_save_source:
        ...
        for i, (x,y) in enumerate(tqdm(source_data)):
            x = x.cuda()
            y = y.cuda()
            f_outputs = netF(x)
        ...
        save_source_path = '/GPFS/rhome/jinyiwang/DiffDA/checkpoints/DA_source/latent.pkl'
        if not os.path.exists(os.path.dirname(save_source_path)):
            os.makedirs(os.path.dirname(save_source_path))
        torch.save(
            {
                'conds': conds,
        ...
        conds_mean = conds.mean(dim=0)
        conds_std = conds.std(dim=0)
        if not os.path.exists(os.path.dirname(save_target_path)):
            os.makedirs(os.path.dirname(save_target_path))
        torch.save(
            {
                'conds': conds,
                'conds_mean': conds_mean,
...
def train_target_rot(args): 
    ...
    print(f'dset_loaders: {len(dset_loaders["target"])}')
    ...
    while iter_num < max_iter:
        ...
        # print(f'inputs_test: {inputs_test.size()}')
        iter_num += 1
        ...
        # print(f'netF(inputs_test).shape: {netF(inputs_test).shape}')
        # print(f'f_outputs.shape: {f_outputs.shape}')
        # exit()
...
def train_target(args):
    ...
    if not args.ssl == 0:
        netR = network.feat_classifier(type='linear', class_num=4, bottleneck_dim=2*args.bottleneck).cuda() # for SSL
...
if __name__ == "__main__":
    ...
    for i in range(len(names)):
        ...
        save_features(args,if_save_source=False)
        # train_target(args)
        args.out_file.close()
